...
import { mediaItems, loraModels } from '@/data/nexapersona-data'

export default function NexaCharactersPage() {
  return (
    <div className="flex flex-col gap-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <p className="text-sm text-foreground-muted">
          Characters are managed centrally in PIP's <Link to="/characters" className="font-medium text-accent hover:underline">Character DNA</Link> module — every generation here draws on that same identity.
        </p>
        <Button asChild>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {characters.map((c) => {
          const generations = mediaItems.filter((m) => m.characterId === c.id).length
          const lora = loraModels.find((l) => l.characterId === c.id)
          return (
              ...
              <Card className="group flex h-full cursor-pointer flex-col overflow-hidden transition-all hover:-translate-y-0.5 hover:shadow-elevation-2">
                ...
                <div className="flex flex-1 flex-col gap-2.5 p-4">
                  ...
                  <div className="mt-auto flex items-center justify-between border-t border-border pt-3 text-xs text-foreground-subtle">
                    ...
                    {lora ? (
                      <Badge variant={lora.status === 'Trained' ? 'success' : 'accent'} className="text-[10px]">
                        <Zap className="size-2.5" />
                        {lora.status}
                      </Badge>
                    ) : (
                      <Badge variant="outline" className="text-[10px]">No LoRA</Badge>
                    )}
                  </div>
                </div>
              </Card>
