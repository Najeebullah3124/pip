...
import { projects, stories } from '@/data/animovia-data'
import { cn } from '@/lib/utils'

export function AnimoviaLayout() {
  const location = useLocation()
  const current = animoviaNavItems.find((item) => (item.end ? location.pathname === item.href : location.pathname.startsWith(item.href)))
    ...
    <div className="flex flex-col gap-6">
      <div className="relative overflow-hidden rounded-3xl border border-border shadow-elevation-2">
        <div
          ...
          style={{
            background:
              'radial-gradient(circle at 12% 15%, rgba(253,164,175,0.5) 0%, transparent 45%), radial-gradient(circle at 88% 10%, rgba(253,230,138,0.4) 0%, transparent 40%), radial-gradient(circle at 55% 100%, rgba(217,70,239,0.35) 0%, transparent 55%), linear-gradient(135deg, #2b0f1f, #4a1230 55%, #2b0f1f)',
          }}
        />
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex items-center gap-3.5">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h1 className="text-2xl font-bold tracking-tight text-white">Animovia</h1>
                ...
                <p className="mt-1 max-w-lg text-sm text-white/70">
                  Turn your characters into illustrated storybooks, narrated readers, and animated book-to-video experiences.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-4">
              <div className="text-right">
                ...
                <p className="text-[11px] text-white/60">projects</p>
              </div>
              <div className="h-8 w-px bg-white/15" />
              <div className="text-right">
                <p className="text-lg font-bold text-white">{stories.length}</p>
                <p className="text-[11px] text-white/60">stories</p>
              </div>
            </div>
          </div>

          <nav className="-mx-1 flex items-center gap-1.5 overflow-x-auto px-1 pb-1">
            {animoviaNavItems.map((item) => (
              <NavLink
                key={item.key}
                to={item.href}
                end={item.end}
